...
get_latest_version() {
  ...
  local tmp_file
  tmp_file="$(mktemp)"
  local url='https://api.github.com/repos/imesh-madushan/xray-core-ghost/releases/latest'
  ...
  url='https://api.github.com/repos/imesh-madushan/xray-core-ghost/releases'
  if curl -x "${PROXY}" -sSfLo "$tmp_file" -H "Accept: application/vnd.github.v3+json" "$url"; then
    echo "get release list success"
  else
    "rm" "$tmp_file"
  ...
  for i in "${!releases_list[@]}"; do
    ...
    url_zip="https://github.com/imesh-madushan/xray-core-ghost/releases/download/${releases_list[$i]}/Xray-linux-$MACHINE.zip"
...
download_xray() {
  echo "Using local Xray binary from /home/ubuntu/prj/xray-core-ghost/xray"
  if [[ ! -f "/home/ubuntu/prj/xray-core-ghost/xray" ]]; then
    echo "error: Local binary /home/ubuntu/prj/xray-core-ghost/xray not found!"
    return 1
  fi
  mkdir -p "$TMP_DIRECTORY"
  cp "/home/ubuntu/prj/xray-core-ghost/xray" "$TMP_DIRECTORY/xray"
  
  echo "Downloading geo files (geoip.dat and geosite.dat)..."
  curl -sL -x "${PROXY}" -R -H 'Cache-Control: no-cache' -o "$TMP_DIRECTORY/geoip.dat" "https://github.com/v2fly/geoip/releases/latest/download/geoip.dat"
  curl -sL -x "${PROXY}" -R -H 'Cache-Control: no-cache' -o "$TMP_DIRECTORY/geosite.dat" "https://github.com/v2fly/domain-list-community/releases/latest/download/dlc.dat"
}

decompression() {
  echo "info: Extraction skipped as local binary is used."
}
